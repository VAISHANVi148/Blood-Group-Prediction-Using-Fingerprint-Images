...
root.title("Detection of Blood Groups")
...
# #
lbl = tk.Label(root, text="Blood Group Detection System", font=('times', 30,' bold '), width=60,height=1,bg="#152238",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        return A
    img = open(fn)    
    myfile = open("C:/Users/vaish/Documents/Detection_of_blood_grp_using_fingerprint_img_100%_Code[1]/Detection of blood grp using fingerprint img 100% Code/testing")
    if img == myfile:
          print(myfile)
...
def update_label(str_T):
    #clear_img()
    result_label = tk.Label(root, text=str_T, width=30, font=("bold", 25), bg='grey1', fg='white')
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='E:/Harddisk/2023 softech project code/23SS162-Blood Cell/Blood Cell Identification 100% Code/Blood Cell Identification 100% Code/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
